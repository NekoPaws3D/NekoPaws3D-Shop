...
function renderProduct(){
  ...
  const inquiry=Boolean(product.priceOnRequest);
  detail.innerHTML=`<article class="detail-card"><div class="gallery"><img id="main-product-image" class="main-product-image" src="${images[0]}" alt="${escapeHtml(product.name)}"><div class="thumbs">${images.map((src,index)=>`<button class="thumb ${index===0?"active":""}" type="button" onclick="setMainImage(${index})"><img src="${src}" alt="Ansicht ${index+1}"></button>`).join("")}</div></div><div><span class="badge">${escapeHtml(product.category)}</span>${product.customizable?'<span class="badge">Personalisierbar</span>':""}${inquiry?'<span class="badge">Individuelle Anfrage</span>':""}<h2>${escapeHtml(product.name)}</h2><p>${escapeHtml(product.description)}</p><div class="price">${inquiry?"Preis auf Anfrage":`${Number(product.price||0).toFixed(2)} €`}</div>${inquiry?'<p class="muted">Teile uns im Kontaktformular Menge, Ausführung und deine Wünsche mit. Du erhältst anschließend ein persönliches Angebot.</p>':`<p class="muted">${Number(product.stock)>0?`Auf Lager: ${product.stock}`:"Derzeit nicht verfügbar"}</p><div class="qty"><button onclick="changeDetailQty(-1)">−</button><span id="detail-qty">1</span><button onclick="changeDetailQty(1)">+</button></div>${product.customizable?'<textarea id="note" placeholder="Farbe, Name oder Wunschmotiv"></textarea>':""}`}<button class="main-btn" ${!inquiry&&Number(product.stock)<=0?'disabled':''} onclick="${inquiry?'openInquiryForm()':'addDetailToCart()'}">${inquiry?'Bestellung anfragen':'In den Warenkorb'}</button></div></article>`;
}
function openInquiryForm(){
  window.location.href=`kontakt.html?product=${encodeURIComponent(product.id)}`;
}
...
document.addEventListener("DOMContentLoaded",()=>{if(window.NEKO_STORE_READY)initProductPage();else window.addEventListener("neko-store-ready",initProductPage,{once:true});});
